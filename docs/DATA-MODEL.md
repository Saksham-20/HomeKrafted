...
| `Payout` | sellerId, amount, periodStart/End, status (`pending`\|`paid`\|**`rejected`** — M15), paidAt?, **reference?/note?/decidedById?/decidedAt? (M15)** | belongs to a `Seller` — `/seller/payouts` and `SellerPayoutsClient` are specialty-agnostic. `reference` is the bank/UPI id the transfer actually moved under: settlement happens **outside this system**, so it is the only link between a row marked paid and a real transfer. **`amount` is the payable figure.** Since the markup commission model (2026-09-16) a marketplace line is *never* deducted here — its fee was already collected from the buyer at checkout (`OrderItem.sellerAmount`) — so `amount` is marketplace net **plus** whatever the legacy streams (laundry/snacks, never migrated to markup pricing) still owe after `computePayoutSplit`'s deduction, applied only to them, only while `PlatformSetting.commissionEnabled` is on. `grossAmount?/commissionAmount?/commissionPct?` record that legacy-only split (0 when the payout has no legacy component); NULL on pre-M37 rows, where `amount` was always gross. `gstAmount?/gstPct?` (2026-09-02) record GST on the fee, same NULL/0 rule. Pending-balance maths subtracts `COALESCE(grossAmount, amount)` over prior payouts as a combined running total, then `allocateClaimedGross` (`server/src/seller/payout-split.ts`) splits that total back into a marketplace share and a legacy share — an estimate, safe in the direction that never double-pays marketplace (see `docs/LAUNCH-READINESS.md` §3b) |
