...
| A place-then-cancel round trip leaves the wallet exactly where it started — for an order that carries a **legacy** cashback snapshot (order cashback was removed 2026-09-19; a new order snapshots 0), cancelling reverses it instead of paying the buyer to cancel | `e2e/orders-lifecycle.e2e-spec.ts` |
...
| Nothing under `client/lib` imports `react`, `next`, `@/components/*` or a non-`@/lib` alias — the native app compiles those exact files, and a shared file reaching for something only the web has fails as a Metro resolution error or two Reacts | `client/lib/shared-boundary.spec.ts` |
...
| The **"All filters" sheet** honours the dialog contract — and the test now actually runs. It looked for a button named `/^Filters/`, which matches nothing in the app (the opener is "All filters"; "Filters" is the dialog's label), and `count() === 0` then **skipped the test**, so the sheet's focus contract went unchecked while the suite stayed green. The category cases in `audit-regressions.spec.ts` had the same locator and now drive `getByRole('radio')` inside the radiogroup named "Category" | `e2e/tests/focus-traps.spec.ts`, `e2e/tests/audit-regressions.spec.ts` |
...
| Order cashback stays off: `CASHBACK_RATE` is `0`, `computeCashback` earns nothing on any basket, the delivery rule beside it is untouched | `server/test/unit/pricing-cashback-off.spec.ts` |
| A **new** order snapshots `cashbackEarned = 0` and writes no `cashback` ledger row on either payment path (wallet, verified Razorpay settlement) — asserted by ledger `category`, not title — while a **legacy** order (`cashbackEarned > 0`) is still credited on both, so a clean-up cannot cost somebody a promised credit | `server/test/e2e/order-cashback-removed.e2e-spec.ts` |
| The client and mobile shared pricing module exports nothing named for cashback (deleted, not zeroed — an import that no longer resolves is a build error), and the loyalty tiers promise no cashback | `client/lib/cart/pricing.spec.ts`, `client/lib/referrals/loyalty-copy.spec.ts`, `mobile/test/checkout-screen.spec.tsx` |
| A failed refresh is not an answer: a 429, a 5xx, a dropped connection, a timeout or a non-JSON body keeps the session and rejects the one request with the status-0 error; only a 401/403 **with no newer token in storage** ends it. Two isolated module copies stand in for two tabs over one storage, one serialising `navigator.locks` and one rotating fake API — a tab adopts a sibling's rotation instead of replaying a spent token (31 tests) | `client/lib/auth/refresh-outcome.spec.ts` |
| A sign-out in another tab signs this one out, a different account signing in re-restores it, a token rotation for the same account does nothing, and a signed-out tab ignores a sign-in elsewhere | `client/lib/auth/cross-tab.spec.ts` |
| The admin queue-badge poll really is throttled to 60 s from when the last poll **started**, and a hidden tab never polls — the handler used to close over `0`, so every window focus fetched `/admin/dashboard` | `client/lib/portal/queue-poll.spec.ts` |
| Refresh reuse detection over an in-memory token table: a replay inside 10 s is refused alone, one beyond it revokes **only its own descendant chain** and never the user's other sessions, a row rotated before `replacedByTokenId` existed revokes nothing beyond itself, and the walk is bounded (19 tests) | `server/test/unit/refresh-token-reuse.spec.ts` |
| Against a real database, rotation writes `replacedByTokenId`, the walk follows real rows, and a stale replay kills one chain while the same user's second device keeps working (`revokedAt` is backdated rather than sleeping through the grace window) | `server/test/e2e/refresh-chain.e2e-spec.ts` |
| A HomeKrafter cannot badge their own work: `tags` sent by a seller is **ignored, not refused** (a create stores `[]`, an update leaves an admin's badge alone), only the admin routes persist it, and no file outside the listing service writes `tags` onto a product — nor does anything under `src/seller/` name `featured`/`featuredRank` | `server/test/unit/listing-tags.spec.ts`, `server/test/unit/product-tags-admin-only.spec.ts`, `server/test/e2e/featured-merchandising.e2e-spec.ts` |
| `PUT /admin/catalog/featured` is a full replacement in one transaction — index + 1 is the rank, anything omitted is unfeatured with its rank cleared, an unknown id is a 400 that writes nothing, and it never touches a listing's moderation note; `unfeature` clears the rank and `feature` leaves it | `server/test/e2e/featured-merchandising.e2e-spec.ts` |
| The default browse leads with featured listings in rank order on **every** path — the SQL fast path, the candidate read and the in-memory comparator agree across a page boundary — and price sorts ignore it, and a featured listing that is not live never surfaces | `server/test/e2e/products-browse.e2e-spec.ts`, `server/test/unit/browse-order.spec.ts` |
| The buyer-side half of featured-first: `compareFeatured` returns 0 on a tie so the caller's own keys decide, `/gifts` pins featured **before** `spreadByMaker` (never handed the featured half), a kitchen is placed by its best featured dish, and the home fallback rails lead with featured even at zero reviews | `client/lib/featured-order.spec.ts`, `client/lib/gift-sort.spec.ts`, `client/lib/kitchens.spec.ts`, `client/lib/home-rails.spec.ts` |
| The card badge chain reads only what an admin set, ends in one honest "Featured" fallback that never shows on a sold-out card, and takes only `tags` and `featured` as input — so it cannot derive a claim from `kind` or `shippingScope` | `client/lib/product/merchandising-badge.spec.ts` |
| The featured screen's pure edits (move, remove, append, cap) never mutate their input and refuse a move off the ends without throwing; the admin client mirrors the server's list order and refusals in mock mode | `client/lib/portal/reorder.spec.ts`, `client/lib/api/admin-featured.spec.ts` |
| A category is one shelf, not a filter: `?category=a,b` opens on the first slug that resolves, an unknown slug is All, the URL carries at most one, an empty browse names its shelf and offers Clear filters and Show all separately | `client/lib/category-sections.spec.ts`, `client/lib/browse-empty.spec.ts`, `e2e/tests/audit-regressions.spec.ts` |
