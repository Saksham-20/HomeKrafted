...
| The **header search is a small circle** until you click it | By design as of 2026-08-11, and worth a try. The header row is capped at 1092px by the page container and the six-item nav fills it, so the search sits collapsed to its magnifier — **click the magnifier (or Tab to it) and the field opens to 420px over the nav**, ready to type. Enter searches; Escape or clicking away closes it. Nothing else in the row should move while it is open. Below 1190px it is the full-width box in the drawer instead. Report it if the field doesn't open, if the wallet/cart icons jump, or if it opens over the wrong thing. |
