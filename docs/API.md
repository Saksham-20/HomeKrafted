...
- `POST /auth/refresh` — `{ refreshToken }` → new `{ accessToken,
  ...
  and replaced in the same operation, and (**2026-09-19**) the old row
  records its successor in `RefreshToken.replacedByTokenId`, written in
  that same transaction. Presenting an already-used (revoked) refresh
  token is rejected with `401` — the reuse-detection signal a
  stolen/replayed token trips — and what else happens depends on how late
  it is:
  - **Within 10 s of the rotation it lost** (`REFRESH_REUSE_GRACE_MS`):
    rejected `401` alone, logged as a same-tab race. That is what two tabs
    refreshing the same token look like, and revoking anything would punish
    the tab that won.
  - **Beyond 10 s:** rejected `401`, and **only that token's own descendant
    chain is revoked** — walked forward through `replacedByTokenId`, bounded
    to 1,000 hops (`REFRESH_CHAIN_MAX_HOPS`), so a thief who rotated first
    loses their working token. **The user's other sessions and devices are
    not revoked.** It used to revoke every active refresh token the user
    had (commit `3f1fc45`), which took an admin's phone and second browser
    down with one stale tab — the 2026-09-19 "admin keeps getting logged
    out" report. The log line moved from `revoked N active session(s)` to
    `revoked N descendant token(s) in its chain; the user's other sessions
    were left alone`.
  - **Rows rotated before this deploy have no `replacedByTokenId`**, so
    replaying one of those revokes nothing beyond itself. That is
    deliberate (guessing a chain from `userId` and `createdAt` cannot tell
    the replayed session from the user's other devices) and it stops
    mattering as those rows age out (7 days).

  Refresh tokens are stored server-side only as a SHA-256 hash
  (`RefreshToken.tokenHash`), never the raw token. `changePassword` and
  `resetPassword` still revoke every session — that is the user asking.
...
| Endpoint | Auth | Notes |
|---|---|---|
| `GET /products` | public | Query params: `q` (free-text, see below), `category`, `occasion`, `vendor` (comma-separated **slugs**, OR-matched within each param, AND across params — mirrors `ShopClient.tsx`'s filter semantics), `dietary` (comma-separated **frontend** tags, e.g. `vegetarian,gluten-free`; the set gained `non-vegetarian` and `contains-egg` on 2026-09-05 — a listing carrying neither veg member matches neither, because absence has never meant non-veg), `featured` (`true`/`false`), `isHamper` (`true`/`false`, **M18** — ready-made gift hampers; three states, since omitting it returns both and a hamper is an ordinary listing that still appears in `/shop`), `minPrice`/`maxPrice` (compared against the `defaultWeightSku`'s price — same basis `ShopClient`'s local `priceOf()` uses), `kind` (`food` \| `craft`, **M20** — omitted returns both, since the split is a browse convenience and a search for "candle" should find one either way), `sort` (`most-loved` default \| `price-asc` \| `price-desc` \| `nearest`, which needs `lat`/`lng`; **since 2026-09-19 `most-loved` orders `featured DESC, featuredRank ASC NULLS LAST, rating DESC, reviewCount DESC, id ASC`** — an admin's featured listings first, in the admin's order, then the ordinary ranking — on all three internal paths: the SQL fast path, the phase-one candidate read that decides which 500 rows survive the cap, and the in-memory comparator, all from `server/src/catalog/browse-order.ts`. `price-asc`, `price-desc` and `nearest`-with-coordinates are unchanged, and `nearest` with no coordinates falls through to the default order as before), `page`/`pageSize` (default 20, **max 500** — raised in 0250bfb; `/shop` and `/gifts` fetch one large page and filter client-side, so the cap is what decides which kitchens exist at all), `includeOutOfRange` (`true` keeps rows outside their kitchen's delivery radius instead of dropping them, each marked `deliverable: false` — a located buyer is shown the whole city and told plainly which half can reach them, rather than given a short list with no explanation; ignored without `lat`/`lng`). Returns `{ items: Product[], page, pageSize, total }`. **Every product payload — buyer, maker and admin — now carries `featuredRank: number \| null`** (`null` = unranked or not featured). Lists only `PUBLICLY_LISTED` statuses (**M22** — an allowlist, not `{ not: "hidden" }`; see `server/src/catalog/moderation.ts`). Ordering breaks ties on `id`, so paging cannot show a row twice or skip one. |
...
| `GET /categories/id/:id` | public | `Category`, looked up **unfiltered** by id — resolves an archived or merged category too, same as `GET /categories/:slug` (both stay unfiltered on purpose, so an old shared link keeps resolving). Only `GET /categories` (the list) drops both. Use `id/:id` for a direct reference (a product's `categoryId`, e.g. the product-page breadcrumb), never for a browse/picker read. |
...
| Endpoint | Auth | Notes |
|---|---|---|
| `GET /reviews?targetType=&targetId=` | public | `targetType`: `product`\|`vendor`\|`service` (`service` is the withdrawn laundry module's target — still readable/writable for delivered legacy bookings, no new surface offers it). Excludes `hidden: true` — same rule `lib/api/reviews.ts` applies. **M37:** `userId` is gone from every review payload — a stable account id on a public route let anybody cross-reference every review one buyer wrote; `userName` remains the public fact. |
...
| Endpoint | Auth | Notes |
|---|---|---|
| `GET /cart` | any authed role | Lazily creates an empty cart. Returns `{ id, userId, updatedAt, items: CartLineDto[], count, subtotal, shippingFee, total, cashbackEstimate }` (**`cashbackEstimate` is always `0` since 2026-09-19** — order cashback was removed; the field stays because an installed native build reads it) — richer than the frontend `Cart` type, see "Response-shape notes for M8.4" below. Every `items[]` entry is resolved fresh via `resolveCartLine` (name/unitPrice/lineTotal/weightLabel/maxQuantity/isHamper, and since 2026-09-15 `kind` (`food`\|`craft`; a hamper line is `craft`), `dietary` (frontend spelling) and `maker: { name, slug, location }` — the coarse public area label, never a pickup address; checkout picks its food or gift layout from `kind`) — `CartItem` itself only stores `productId`+`sku`/`hamperId`+`quantity`, no price. |
| `POST /cart/items` | any authed role | Body `{ productId, sku, quantity? }` (default 1). Adds, or increments an existing `productId`+`sku` line — mirrors `CartContext.addItem`. `400` if the resulting quantity would exceed `WeightOption.stock`.  **2026-09-14: one basket, one maker.** Answers **409 `CART_OTHER_MAKER`** when the cart already holds a different `Vendor`'s product, with that maker's name in the message so the client can offer to empty the basket — see `server/src/cart/one-maker-cart.ts`. Checked before the stock arithmetic. `POST /cart/hamper-items` carries the same gate (its line stores `hamperId`, not `productId`, so it was the way round), and refuses a hamper spanning two makers. **2026-09-15:** a food listing answers **409 `FOOD_COMING_SOON`** while `foodOrdersOpen` is off (checked before the maker rule). |
...
| Endpoint | Auth | Notes |
|---|---|---|
| `POST /orders` | any authed role | Creates an order from the caller's **current `Cart`** — see "Server-authoritative pricing" below. Body: `{ defaultAddressId?, shipments?: [{addressId, deliveryDate?}], gift?: {recipientName?, recipientAddressId?, hidePrice?, message?}, paymentMethod: "wallet"\|"razorpay"\|"cod" }`. **M50 — both recipient fields are optional**: `recipientAddressId` is the switch that makes the parcel ship somewhere else, and without it `gift` records a **message card on an order the buyer keeps** and hands over themselves, which the product page's "Make it a gift" block can now ask for. When it *is* present it must be one of the caller's own saved addresses — "ship to someone else" means the recipient's address is in your address book, the same FK shape `Order.giftRecipientAddressId` requires; the mock checkout's synthetic `"gift-recipient"` id doesn't carry over (flagged for M8.4 below). `400` on an empty cart or a missing shipping address for some line; `404` if any resolved address isn't owned by the caller; `409` on a stock race lost inside the transaction. Clears the cart on success. **Accepts `Idempotency-Key` (2026-08-07 audit).** Until then it did not, and nothing on the server stopped a repeated submit from creating a second order with its own stock decrement and its own payable total — the only guard was `CheckoutClient`'s `placing` React state, which is not a lock. Three clicks in one task produced three orders and three wallet debits. The key check runs **before** the cart validation, so a sequential replay (a refresh, a retried request) returns the original order instead of failing "Cart is empty" for an order that in fact succeeded. | **2026-09-17 — `deliveryMode?: "standard" | "isb-campus"`** (absent = `standard`, so every older client is unchanged). `isb-campus` is hand-delivery onto the ISB campus: `shippingFee` is forced to **0** whatever `deliveryFee` is set to, no `Consignment` is ever booked, and **the server writes the destination** from `ISB_CAMPUS_ADDRESS`, ignoring `defaultAddressId` and every per-item address (including a gift recipient's) — otherwise free hand-delivery is claimable for an address nobody is going to walk to. `campusDrop` (where on campus, 2–120 chars) is **required** for that mode, refused with the sentence naming the box; `campusPhone` is optional and falls back to the account's phone, refused if there is neither. One `ISB campus` Address row per buyer, updated with each order's drop detail. See `server/src/common/delivery/isb-campus.ts`.
...
| `POST /orders/:id/pay` | any authed role, own order only | **M8.2.** Completes the `pending-payment -> placed` seam for a `paymentMethod: "wallet"` order: debits the wallet for `order.total` (read fresh from the DB), transitions the order to `placed` — one atomic transaction — and credits `order.cashbackEarned` only if that stored snapshot is non-zero (**a pre-2026-09-19 order, or one still pending payment at the deploy; every newer order snapshots `0` and no credit row is written**). `402` (`INSUFFICIENT_BALANCE`) if the wallet can't cover it, order left untouched. `409` if the order isn't `pending-payment` (already paid/cancelled) or `400` if its `paymentMethod` isn't `"wallet"`. Accepts an `Idempotency-Key` header (see "Idempotency" below) — safe to retry. |
...
| `POST /orders/:id/refund` | `@Roles('admin')` | **M8.2.** Credits the order owner's wallet for `order.total` (`category: "refund"`) and sets `refundStatus: "refunded"`. `404` unknown order, `409` if the order was never paid (`pending-payment`). Idempotent: a second call (same `Idempotency-Key`, or none at all — the "already refunded" check alone catches it) returns the same result without a second credit. **Writes an `AdminAuditLog` row (`order.refund`) as of 2026-08-15** — it wrote none before, for the same module-cycle reason as `POST /wallet/adjust` above, while its twin `POST /admin/orders/:type/:id/refund` was always audited. Logged after commit, and only on the pass that actually refunds, so a replay logs nothing. **Still notifies nobody** — see `docs/SHOPIFY-PARITY.md` P0.3. |
...
stock). `subtotal`/`shippingFee`/`total` use the same `computeShipping`
rule as `client/lib/cart/pricing.ts` (since 2026-09-15 the delivery fee and free-delivery threshold come from `PlatformSettings.deliveryFee`/`freeDeliveryThreshold` — default ₹0 and ₹999 — rather than the former flat ₹49 under ₹999; free
at/above it) — ported to `server/src/common/pricing/pricing.util.ts`.
**`cashbackEarned` is `0` on every order created since 2026-09-19**: the
flat 5% order cashback was removed (owner: "remove cashback from
wallet"), and `CASHBACK_RATE` in that file is `0` rather than deleted
because the field is a *snapshot* taken at checkout — the credit
(`payWithWallet`, the Razorpay webhook) and the reversal (cancel, refund)
both read the snapshot, so an order that was quoted a cashback before the
removal is still credited if it is paid and reversed if it is cancelled,
symmetrically. The client and mobile shared `cart/pricing` no longer export
`CASHBACK_RATE`/`computeCashback`.
...
- **`paymentMethod: "wallet"`** → `POST /orders/:id/pay` (above): debits
  the wallet and transitions to `placed`, atomically (a legacy order's
  stored cashback is credited in the same transaction; a new order has none).
...
- **`paymentMethod: "razorpay"`** → `POST /payments/razorpay/order` (see
  ...
  `pending_payment -> placed` (and credits a legacy order's stored cashback, if it has one).
...
- `GET /cart`'s response is richer than the frontend `Cart` type
  ...
  `count`/`subtotal`/`shippingFee`/`total`/`cashbackEstimate` (always `0` since 2026-09-19). M8.4 can
...
| Endpoint | Auth | Notes |
|---|---|---|
| `GET /wallet` | any authed role | `{ id, userId, balance, pendingCashback, lifetimeSaved, payWithWalletDefault, updatedAt }` (**`pendingCashback` and `lifetimeSaved` are legacy since 2026-09-19** — the first was never written by anything, the second is a frozen running total of the order cashback that used to be credited, and no web screen renders either; they still return because an installed native build reads them) — lazily creates a zero-balance wallet if none exists yet (shouldn't happen for a real account; `auth.service.ts` creates one at registration). |
...
| `POST /wallet/adjust` | `@Roles('admin')` | Manual credit/debit: `{ userId, direction: "credit"\|"debit", amount, reason }`. The one endpoint where a caller-supplied `amount` is intentional — gated to admins, `reason` required (becomes the ledger row's `title` for audit), `category: "adjustment"`. **Writes an `AdminAuditLog` row (`wallet.adjust`) as of 2026-08-15** — it wrote none before, because `WalletModule` could not import the audit writer without a module cycle with `AdminModule`. `AdminAuditModule` (`src/admin/audit.module.ts`) exists to break that cycle. The row is written after the transaction commits and only on the pass that actually posts the ledger entry, so an idempotent retry does not log a second adjustment. |

There is deliberately **no** `POST /wallet/topup`, `/wallet/pay`
or generic `/wallet/refund` endpoint — each would
...
| `POST /payments/razorpay/verify` | any authed role | Standard Checkout's "verify payment signature" step (2026-09-15), called by the browser from Checkout's success handler. Body: `{ razorpay_order_id, razorpay_payment_id, razorpay_signature }`, all required (`400` if missing). Checks, before anything is written: HMAC-SHA256 of `order_id\|payment_id` keyed with `RAZORPAY_KEY_SECRET`, constant-time (`400`, nothing marked paid, on mismatch); the `RazorpayOrder` belongs to the caller (`404` otherwise); then **`GET /v1/payments/:id` at Razorpay** must report this order, this amount and `status: captured` — a valid signature only proves the payment belongs to the order. Returns `{ verified: true, status: "captured" }` once applied, or `{ verified: true, status: "pending" }` for an `authorized`-not-yet-captured payment, which the webhook finishes. Applies the money through the **same** transaction and `WebhookEvent` dedup key (`payment.captured:<paymentId>`) as the webhook, so the two racing apply it exactly once. The webhook stays the backstop for a buyer who closes the tab first. `400` on a server with placeholder keys. |
| `POST /payments/razorpay/webhook` | **public** (`@Public()`) | Razorpay's server calls this, not a signed-in shopper. Verifies `X-Razorpay-Signature` (HMAC-SHA256 over the **raw** request body, keyed with `RAZORPAY_WEBHOOK_SECRET`) before touching any state — an invalid/missing signature is `400`, nothing evaluated further. Only acts on a `payment.captured` event; every other event type is acknowledged `200` as a no-op. On a valid `payment.captured`: looks up the `RazorpayOrder` row by `payload.payment.entity.order_id` (never trusts the payload's amount) and either credits the wallet top-up (+3% bonus above ₹2,000, mirroring `client/lib/wallet/WalletContext.tsx`'s `TOPUP_BONUS_THRESHOLD`/`RATE`) or transitions the linked `Order` `pending-payment -> placed` (crediting that order's stored cashback snapshot, which is `0` for every order created since 2026-09-19 — only a legacy order still gets a credit) — depending on that row's `purpose`. Deduplicated by `(event, paymentId)` via a `WebhookEvent` unique-insert — a redelivered event is acknowledged `200` as a duplicate, never reapplied. |
...
(while the flat order cashback existed, until 2026-09-19) through
`computeCashback` as ~5% credited to an account auto-created for a
...
| `PATCH /admin/sellers/:id/coords` | `@Roles('admin')` | **M36.** Body: `{ lat, lng, location? }`. Moves a kitchen to where it actually is — the correction step for a pincode centroid that was not close enough. `Vendor.lat`/`lng` decides which buyers can see a storefront at all, and the bundled centroid is trustworthy for only 44% of Indian pincodes, so this endpoint is what makes approving nationally safe. Audited with before/after (`vendor.set_coords`). **A seller-facing sibling exists since 2026-08-18** (`PATCH /seller/profile/coords`, see the seller profile section) — plausibility-checked, badge-clearing and audited; this admin endpoint remains the unconstrained correction path, the only one that may move a pin outside the stated pincode. |
...
| Endpoint | Notes |
|---|---|
| `GET /seller/profile` | The caller's own profile — everything the public one has, plus `fssaiNumber`, `fssaiExpiry`, `verifiedAt`, `verificationNote`, `completion: { percent, missing[] }`, the private `pickup` address (M36b) and `pin: { lat, lng, pincode? }` — the exact coordinates behind delivery-distance filtering, own-view only (the public payload carries the ~1.1 km rounded point, M36). |
...
| `PATCH /seller/profile/coords` | **2026-08-18.** Body: `{ lat, lng }`. The kitchen pins its own exact spot — the person standing in it with a GPS fix knows where it is better than a pincode centroid trustworthy for 44% of pincodes. Three guardrails keep the M36 abuse case closed: the pin must land within the kitchen's own pincode (centroid + measured `spreadKm` + 10 km; pre-M36 kitchens: 25 km of their curated area — anchored to the *stated* place, never the current pin, so repeated moves cannot walk a storefront anywhere) or it is a 400 naming the distance; an accepted pin **clears `addressVerified`** in the same transaction (the M36c rule — an admin verified a place, not a claim); and it is audited (`vendor.set_coords_self`). Buyer exposure unchanged: `mapVendor` rounds every public payload to ~1.1 km whoever set the pin. No `location` override, unlike the admin endpoint — a pin move cannot also rewrite the storefront's area label. |
...
| `POST /seller/listings` | Body mirrors `client/lib/api/seller.ts`'s `SellerListingInput`: `{ name, categoryId, occasionIds?, dietary?, prepTimeMins?, description, isPackaged, isHamper?, kind?, shippingScope?, isSnack?, cashbackPct, tags?, imagePath?, imagePaths?, weightOptions: [{sku,label,price,mrp,stock}], defaultWeightSku }`. **2026-09-19 — `tags?` is accepted but IGNORED for a HomeKrafter** (Bestseller/New/Festive/Curated are merchandising, an admin's to give): a create stores `[]`, and a `PATCH` leaves the column untouched, so an admin-set badge survives the maker's next price edit even though every save posts the form's `tags` back. Only `POST`/`PATCH /admin/catalog/products` persist `tags`. The field is **kept on the DTO rather than removed** because `forbidNonWhitelisted` would turn a removed field into a `400` for every web bundle and native build already shipped (the M22 rule about narrowing a request value); the rule lives in `server/src/seller/listing-tags.ts`. `cashbackPct` is likewise an **inert field kept only so existing values round-trip** — order cashback was removed 2026-09-19 and nothing computes or renders from it. **2026-09-05:** `dietary` accepts `non-vegetarian` and `contains-egg` alongside the existing five; the listing form makes the veg/non-veg pair exclusive, but the server does not enforce that — `dietOf()` resolves a contradictory pair to non-veg client-side, the reading that cannot mislead. `prepTimeMins` (optional int, 0–43200) is how much notice **this listing** needs and drives the buyer's "Pre-order" badge; omit it for "not stated" — the server writes NULL and no badge is drawn. It never re-queues a listing for moderation (M22: a material change is name, description, category or photo, not a schedule). **M20 section flags:** `kind` (`food\|craft`, default `food`) decides which vertical the listing appears in — `/gifts` is the catalogue filtered on `craft`. `shippingScope` (`local\|national`, default `local`) is deliberately *not* derived from `kind`: a `national` listing skips the delivery-radius gate entirely, and a kitchen posting pickles across India is a real case. `isSnack` (default `false`) adds it to the WhatsApp snacks menu without removing it from the shop. All three are optional so a pre-M20 client that sends none of them behaves exactly as before. Validates `categoryId`/`occasionIds` exist and every `weightOptions[].sku` is globally unique (`409` on clash — `WeightOption.sku` is a unique column) before inserting. **2026-09-17 — photos are a list.** `imagePaths?: string[]` (max 6; index 0 is the primary: the product card, the OpenGraph image, the JSON-LD image) supersedes the single `imagePath?`, which is **still accepted** so a native client already in somebody's hands keeps working; a request carrying both is answered by the list. `server/src/seller/listing-photos.ts` owns the normalising (trim, drop blanks, drop duplicates keeping first position, truncate at 6) and the M22 question: changing the primary, or adding/removing a photo, re-queues a live listing; reordering the ones below the primary does not. An empty array means "remove the photos" and is distinct from omitting the field, which leaves the rows alone. `slug` is server-generated from `name` (+ a random suffix on collision). **2026-09-14:** `allergens?: string[]` is finally stored. It was on this DTO and on the client's `Product` type from the start, with no column behind it — a declared allergen was accepted and dropped. Values are the buyer-facing labels (`ALLERGEN_OPTIONS` in `client/lib/sell/listing-families.ts`) plus the sentinel **"None of these"**, which is stored like any other value because an empty array means both "no allergens" and "we never asked". `[]` is never a safety claim in either direction; `splitAllergens` is the one place the sentinel is taken back out, and it must never reach a "Contains ___" line. |
...
Where a HomeKrafter says *"there is no shelf for what I make"*. Before
this the shelf and occasion pickers said "ask us to add it" and there was
nobody to ask.

**These routes create neither a `Category` nor an `Occasion`.** They file
a `TaxonomySuggestion`; an admin approving it is what mints the real row
(`POST /admin/taxonomy-suggestions/:id/approve`). That is not permissions
hygiene — those two tables are a shared vocabulary the whole catalogue
browses by, and one anybody can append to stops being one ("Pickles",
"Pickle" and "Achaar" as three half-empty shelves nothing can merge). The
`Occasion` invariant is unchanged and still pinned by
`server/test/unit/occasion-admin-only.spec.ts`.

| Endpoint | Notes |
|---|---|
| `GET /seller/taxonomy-suggestions` | Mine, newest first, capped at 50. Includes decided ones, so a rejection's `decisionNote` is readable. |
| `POST /seller/taxonomy-suggestions` | Body: `{ kind: "category" \| "occasion", name, group?: "food" \| "craft", note? }`. `name` is 2–48 chars and whitespace-collapsed; `group` is the form's own "something to eat / something to keep" answer and is **ignored for an occasion**, which has no side of the catalogue. Two deliberate **409**s, each carrying the sentence that says what to do instead: the name already exists as a real row (*"“Bakery” is already on the list — pick it rather than asking for it"*), or this person already has a pending ask under that name. `vendorId` is taken from the caller's own session, never the body. |
...
| Endpoint | Notes |
|---|---|
| `PATCH /admin/users/:id/admin-access` | **M47.** Body: `{ isAdmin, scopes }`. Makes somebody a sub-admin, changes which sections they cover, or removes it. One route rather than two, because they are one decision — an admin with no sections can reach nothing. Four guardrails, each a 400 with its own sentence: **you cannot change your own access** (no self-elevation, no self-lockout), **the last admin holding `users` cannot lose it** (that is the scope that grants scopes), **an admin with no sections is refused**, and **removing access clears the scopes** rather than leaving a demoted row that still lists sections. Audited with before/after. Scope: `users`. |
...
| `POST /admin/sellers/applications/:id/approve` | `409` if already `approved`/`rejected`. Otherwise, **one atomic transaction**: (1) finds-or-creates the applicant's `User` account (reuses an existing account by email if one exists, upgrading `consumer` → `seller`; refuses with `409` if that account's role is neither `consumer` nor `seller` — an `admin`/`rider` account cannot be silently promoted or demoted; otherwise mints a fresh `role: "seller"` account with `authProviders: ["phone"]` and no password — phone-OTP is its login path — plus a `Wallet` + `LoyaltyAccount`, same recipe `AuthService.verifyOtp`'s first-time-phone signup uses); (2) creates a `Vendor` storefront from the application's business details (`SellerApplicationCategory` → `VendorType`, `"other"` → `"maker"`); (3) creates the `Seller` row (`type: "maker"`, `status: "approved"`) pointing at it; (4) marks the application `approved`. Unlike the M11a frontend mock (which pointed `Seller.userId` at a synthetic placeholder id), this is a real FK — a live `User` row must exist, so one is provisioned right here. Returns `{ application, seller, vendor, invite, signIn }`. Audited (`seller_application.approve`). **M32:** the account is now created *with* an issued password (`mustChangePassword: true`), not none — see `POST /admin/sellers/:id/temp-password`. **`signIn` is omitted, not returned empty, when step (1) reused an account that already had its own password** — that credential is never rotated and its sessions are never revoked. **The phone-collision guard runs only inside step (1)'s create-a-new-`User` branch** (i.e. only once no account has matched by email) and refuses with a named `409` when the application's phone already belongs to a different account; the reuse branch never writes `User.phone` and is never refused over it, even if the reused account's own stored phone differs from the one on this application and that number happens to belong to someone else entirely. |
...
| `POST /admin/catalog/products` | **M44.** Body: the seller `CreateListingDto` plus optional `vendorId`. Creates a listing; **`vendorId` omitted means the platform's own "Homekrafted" storefront** (resolved by slug, 404 with a fixable sentence if unseeded). Passing a HomeKrafter's `vendorId` is the assisted-onboarding path — an operator types a kitchen's products up for them, onto that kitchen's storefront. Goes **straight to `active`** with the admin in `moderatedById`/`moderatedAt` (`initialAdminSubmission`): the reviewer wrote it, so queueing it would queue a listing for its own author. **This route and the `PATCH` below are the only ones that persist `tags`** (Bestseller/New/Festive/Curated — 2026-09-19; the seller routes ignore the field). Audited (`product.create`). |
| `PATCH /admin/catalog/products/:id` | **M44.** Body: the seller `UpdateListingDto`. Full-record edit for any vendor's listing. **Does not re-queue** — an operator fixing a typo must not take a live listing off sale to do it, which is the same failure `requeueOnEdit` avoids on the seller side. The admin screen for this shipped in M11b against a client wrapper that wrote to an in-memory array and returned, so every admin edit since was silently discarded. `tags` is written only when the body carries it (an explicit `[]` removes the badges). Audited (`product.update`). |
| `PATCH /admin/catalog/products/:id/moderate` | Body: `{ action: "approve"\|"reject"\|"hide"\|"unhide"\|"flag"\|"unflag"\|"takedown"\|"feature"\|"unfeature", reason?: string }`. **M22:** `approve` resolves a `pending` listing *and* restores a hidden/flagged one (both write `active`); `reject` writes `rejected`. **`reason` is required (min 10 chars) on `reject`/`hide`/`takedown`/`flag`** — 400 without it — and is stored on `Product.moderationNote`, shown verbatim to the HomeKrafter and sent to them by every channel their `account` preferences allow. An allowing action clears the note. `feature`/`unfeature` toggle `Product.featured` only and deliberately leave the note and decision stamp alone. **2026-09-19:** `unfeature` also sets `featuredRank` to `NULL` (a listing that leaves the set must not carry a stale place back in the next time it is featured); `feature` leaves the rank as it is — `NULL` means "featured but unranked", which sorts after every ranked listing. Ordering is done on `PUT /admin/catalog/featured` below, not here. Every status change also stamps `moderatedById`/`moderatedAt`. Audited (`product.<action>`) with `{ from, to, reason }`. |
| `GET /admin/catalog/featured` | **2026-09-19.** `{ items: (Product + vendorName + categoryName)[], total }` — every listing with `featured = true`, ordered `featuredRank` ascending **NULLS LAST**, then `name`, then `id` (a stable order among the unranked). **Includes featured listings that are not live**: `moderationStatus` and `isAvailable` ride on each row, because a listing an admin later hid still occupies a place in the set and the screen has to show it to let the admin take it out. Buyer-side price mapping (`mapProduct`). Scope `catalog`. |
| `PUT /admin/catalog/featured` | **2026-09-19.** Body: `{ productIds: string[] }` — the **complete** featured list, in order. Max 100 ids (`MAX_FEATURED`; a longer list is a `400`, never truncated), each a non-empty string; duplicates collapse to their first position; an empty list means "feature nothing". **A full replacement in one transaction:** array index + 1 becomes `featuredRank`, and anything featured and not listed becomes `featured = false, featuredRank = null`. An id that matches no listing is a `400` naming up to 10 of them ("…and N more"), checked inside the transaction before anything is written. **Merchandising, not moderation:** it writes `featured` and `featuredRank` only and never touches `moderationStatus`, `moderationNote` or `moderatedAt` — featuring a flagged listing must not erase why it was flagged (M22). Featuring does not require the listing to be live; a buyer only sees it once it also passes `PUBLICLY_LISTED`. Audited as `catalog.featured_set` (`targetType: Product`, `metadata: { before, after }` — the two id lists). Returns the fresh list, same shape as the `GET`. Scope `catalog`. |
...
| `PATCH /admin/orders/marketplace/:id/pickup-spot` | Scope `orders`. Where to collect an ISB campus order, in the operator's own words (`{ message }`, 3–300 chars, trimmed before the length check). Writes `Order.pickupSpot`, audits `order.set_pickup_spot` with before/after, and **notifies the buyer — email included** (the ordinary `order` category, where email is on by default), because checkout promised exactly that message instead of asking them where on campus. **Campus orders only** — a standard order has a street address and nothing to collect from, so it is refused with that sentence rather than storing a spot nobody is sent. Re-sending replaces the spot and notifies again: a gate closes, a rider is rerouted, and a silent correction is worse than a second message. Returns the updated order. |

### Wallet oversight (`server/src/admin/wallet.controller.ts`)

| Endpoint | Notes |
|---|---|
| `GET /admin/wallet` | Platform-wide totals plus one page of per-user balances: `{ totalLiability, walletCount, totalLifetimeSaved, balances, page, pageSize, total }`. Query: `page`, `pageSize` (default 25, max 100). **The three totals are aggregates over every wallet and are never narrowed by the page** — a liability figure that only totalled the rows on screen would quietly mean something else. `{ totalLiability, walletCount, totalLifetimeSaved, balances: [{userId,userName,walletId,balance,pendingCashback,lifetimeSaved,transactionCount}] }` — every wallet, balance descending. `totalLifetimeSaved`, `pendingCashback` and `lifetimeSaved` are **legacy since 2026-09-19** (order cashback was removed): a frozen running total of cashback already credited, and a column nothing has ever written. They still return for installed native builds. |
...
| `POST /admin/collections/occasions` | **M43.** Body: `{ name, celebratedOn?, tagline?, imageSrc? }`. **The only route in the product that creates an `Occasion` directly** (M50 added a second admin path — approving a HomeKrafter's suggestion — which is also under `/api/v1/admin` and is the same rule), **and the only one there may be** — it sits under `/api/v1/admin`, where `RolesGuard` is fail-closed, and there is deliberately no `/seller/*` equivalent: a HomeKrafter tags a listing with an occasion, they do not mint one. A same-name occasion (case-insensitively) is **409**, not silently de-duplicated — handing back the existing row would make the admin believe the date and tagline they typed had been saved onto it. `slug` and `initial` are derived; an absent `celebratedOn` stores evergreen. Audited (`occasion.create`). Pinned by `server/test/unit/occasion-admin-only.spec.ts`. |
...
### Taxonomy suggestions (M50) — `server/src/admin/taxonomy.controller.ts`

`@RequireAdminScope('catalog')`. Approving is what mints the real
`Category`/`Occasion` row — see the seller-side section above for why the
ask and the create are separate.

| Endpoint | Notes |
|---|---|
| `GET /admin/taxonomy-suggestions?status=` | `{ items, pendingCount }`. Ordered pending-first and **oldest-pending first inside it**: there is a HomeKrafter waiting behind every one, and newest-first is the order where the first ask waits longest. An unrecognised `status` reads as no filter (shows more, not fewer). `pendingCount` is the whole queue, deliberately not narrowed by the filter. |
| `POST /admin/taxonomy-suggestions/:id/approve` | Body: `{ name?, celebratedOn?, tagline? }`. `name` renames it on the way in — that is the point of a human step: the person who can see the whole list turns "achaar" into "Pickles & Preserves" rather than refusing somebody who used their own words. A category lands on the `group` the asker's own form recorded; an occasion takes an **absolute** `celebratedOn` or none (never a recurrence rule — CLAUDE.md, M16). The duplicate check re-runs **against the final name**, and a clash is a **409** naming the existing row rather than a silent de-duplication. `400` if it was already decided. Audited (`taxonomy.suggestion.approve`). Notifies the asker. |
| `POST /admin/taxonomy-suggestions/:id/reject` | Body: `{ reason }`, `@MinLength(10)`. The reason reaches the HomeKrafter **verbatim** on every channel their `account` preferences allow (the M22 rule) — it is the only thing telling them whether to pick an existing shelf or ask again differently. `400` if already decided. Audited (`taxonomy.suggestion.reject`). |
...
| `getFeatured()` | `Product[]` | `GET /api/v1/products`, filtered client-side on `featured`. **No page calls it today**: `featured` means "leads the default browse, in `featuredRank` order", and the buyer surfaces read it through `client/lib/featured-order.ts` — see the `GET /products` sort note and `PUT /admin/catalog/featured`. |
...
`WalletContext`'s `topUp`/`pay`/`refund` client methods swap
at **M8.4** to: `topUp` → `POST /payments/razorpay/order` (`purpose:
"topup"`) + the Razorpay Checkout SDK; `pay` (at marketplace checkout) →
`POST /orders/:id/pay`; `refund` has no direct client
call anymore — it only ever happens server-side (admin
refund) and just shows up in the next `GET /wallet/transactions` poll.
(`earnCashback` was a fourth method until 2026-09-19, when order cashback
was removed and it went with it.)
...
- **Verified live** (headless browser, seeded Postgres): sign-in (auto +
  ...
  debited — and, at the time, cashback credited; order cashback was
  removed 2026-09-19), laundry booking (atomic wallet debit),
