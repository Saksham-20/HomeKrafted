...
/**
 * Cart (M3) — reads straight from `useCart()`; there is no unique server
 * data to fetch, since the cart store already resolves line pricing from
 * the catalog it loads internally.
 *
 * Split out of `app/cart/page.tsx` in M15: a `"use client"` route file
 * can't export `metadata`, and the cart needs a `noindex` and a real tab
 * title. This is the same server-wrapper/client-component split
 * `Header.tsx` → `HeaderClient.tsx` uses everywhere else.
 */
