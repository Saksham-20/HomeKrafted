...
export interface AccountOverviewClientProps {
  /** Seeded order-history count, server-fetched (`getOrderHistory()`) — see that function's comment on why this is the reliable count vs. a live client re-fetch. */
  orderCount: number;
  addressCount: number;
}

/**
 * Account overview (M7a) — greeting, a wallet-balance snapshot (real
 * client state via `useWallet()`, same as the header chip), and a quick-
 * link grid into the other four sections. Order/address counts come from
 * the server-fetched props above; wishlist count and wallet balance are
 * read live so they're always current without a re-fetch.
 */
export function AccountOverviewClient({ orderCount, addressCount }: AccountOverviewClientProps) {
  const { user } = useAuth();
